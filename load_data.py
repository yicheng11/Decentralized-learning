...
class Generator(object):
    ...
    def group(self):
        ...
        for datapoint in self.trainset:  # pylint: disable=all
            _, label = datapoint  # Extract label [tensor, label number]
    ...
    def generate(self, path):
        ...
        return self.trainset # {label:[datapoints]}
...
class Loader(object):
    ...
    def __init__(self, config, generator):
        ...
        self.config = config
        self.trainset = generator.trainset # 這邊的 training set 是 dictionary?
        self.testset = generator.testset
        logging.info('Test size: {}'.format(len(self.testset)))
    ...
    def extract(self, label, n):
        if len(self.trainset[label]) > n: # {label:[datapoints]}
    ...
    def get_partition(self, partition_size):
        ...
        dist = dists.uniform(partition_size, len(self.labels)) # return each label distribution list
    ...
    def get_trainset_list(self):
        # Return the trainset in list
        trainset_list = []
        for key in self.trainset:
            trainset_list.extend(self.trainset[key])
