...
class Gossip_RL_pca(object):
    ...
    def __init__(self, config):
        self.config = config
        self.band_degree = \
        [6, 8, 6, 4, 7, 8, 7, 5, 6, 7, 5, 4, 5, 5, 5, 7, 4, 8, 4, 7, 4, 5, 5, 8, 5, 4, 6, 8, 7, 8, 4, 7]
        # 計算每個 Nond 最多可用的 degree
        self.data_dist = {} # 紀錄各個 ID 的 data distribution
        self.mean_loss = []
        self.BATCH_SIZE = 128
        self.EPS_START = 0.9
        self.EPS_END = 0.05
    ...
    def rl(self):
        ...
        for i_episode in range(self.NUM_EPISODE):
            ...
            self.boot()
            self.run()
            with open('reward_history.txt', 'a+') as R:
                R.write(str(self.reward_sum)+'\n')
                R.close()
        logging.info('nmsl complete!!!!!!!')
    ...
    def check_band_degree(self, action):
        ...
        if self.current_matrix[vertex[0][0]][vertex[0][1]] and self.current_matrix[vertex[0][1]][vertex[0][0]]:
            return True
        #### 檢查"加入此 action 後" degree 是否還滿足限制條件
    ...
    def make_clients(self, num_clients):
        ...
        # 設計資料分布參數
        clients = []
        label_dist = [5,2,1,6,4,3,4,2,3,2]
        label_count = 0
        ran = self.config.randomnodes
            ...
            if not IID:  # Configure clients for non-IID data
                if self.config.data.bias:
                    ...
                    if ran == True:
                        ...
                        flag = True
                        while(flag):
                            if label_dist[label_count] != 0:
                                label_dist[label_count] = label_dist[label_count] - 1
                                flag = False
                                pref = labels[label_count]
                            else:
                                label_count = label_count + 1
                    # Assign preference, bias config
                    if (self.config.random_non_iid):
                        ran_bias = round(random.uniform(0.1,0.9), 1)
                        new_client.set_bias(pref, ran_bias)
                    else:
    ...
    def run(self):
        ...
        if self.config.data.IID:
            D = "IID"
        elif ((self.config.data.IID == False) and (self.config.random_non_iid == False)):
            D = self.config.data.bias['primary']
        else:
            D = 'random'
        if self.config.readowngraph == True:
            logging.info('Read own graph')
        else:
        ...
        rl_path = './RL_accuracy/EP_{}'.format(self.i_episode)
        if not os.path.isdir('./RL_accuracy'):
            os.mkdir('./RL_accuracy')
        if not os.path.isdir(rl_path):
            os.mkdir(rl_path)
        with open(rl_path+'/action.txt', 'w+') as a:
            a.write('')
            a.close()
        with open(rl_path + '/_top.txt', 'w+') as gg:
            gg.write('')
            gg.close()
        with open(rl_path + '/_less.txt', 'w+') as gg:
            gg.write('')
            gg.close()
        with open(rl_path + '/_t25.txt', 'w+') as gg:
            gg.write('')
            gg.close()
        with open(rl_path + '/_t50.txt', 'w+') as gg:
            gg.write('')
            gg.close()
        with open(rl_path + '/_LOSS.txt', 'w+') as gg:
            gg.write('')
            gg.close()
        with open(rl_path + '/_avg.txt', 'w+') as gg:
            gg.write('')
            gg.close()
    ...
    def round(self, num_round):
        ...
        accuracy_list = []
        rsample = random.sample(range(0, N), 4)
        ...
        less_accuracy = min(accuracy_list)
        mean_accuracy = sum(accuracy_list)/len(accuracy_list)
        accuracy_list.sort()
        top25 = sum(accuracy_list[:(len(accuracy_list)//4)]) / (len(accuracy_list)//4)
        top50 = sum(accuracy_list[:(len(accuracy_list)//2)]) / (len(accuracy_list)//2)
        ...
        rl_path = './RL_accuracy/EP_{}'.format(self.i_episode)
        with open(rl_path + '/_top.txt', 'a+') as acc:
            acc.write(str(highest_accuracy))
            acc.write('\n')
            acc.close()
        with open(rl_path + '/_less.txt', 'a+') as acc:
            acc.write(str(less_accuracy))
            acc.write('\n')
            acc.close()
        with open(rl_path + '/_t25.txt', 'a+') as acc:
            acc.write(str(top25))
            acc.write('\n')
            acc.close()
        with open(rl_path + '/_t50.txt', 'a+') as acc:
            acc.write(str(top50))
            acc.write('\n')
            acc.close()
        with open(rl_path + '/_LOSS.txt', 'a+') as acc:
            acc.write('{:.6f}'.format(sum(self.mean_loss) / len(self.mean_loss)) + '\n')
            acc.close()
        with open(rl_path + '/_avg.txt', 'a+') as acc:
            acc.write(str(mean_accuracy))
            acc.write('\n')
            acc.close()
    ...
    def read_graph(self):
        ...
        if (self.num_round == 1): # number of round start from 1
            ...
            self.g_original_matrix = self.read_graph_as_matrix(path) # 把txt轉換成 graph matrix
            ...
            with open('./RL_accuracy/EP_{}'.format(self.i_episode) + '/action.txt', 'a+') as a:
                a.write(str(self.action) + '\n')
                a.close()
        ...
        logging.info('Created RL graph RRRRRRRRRRRRRRRRR')
    ...
    def create_action_table(self):
        ...
        action_matrix = copy.deepcopy(self.g_original_matrix) # 紀錄 pair 對應到的 action
    ...
    def calculate_degree_constrain(self):
        bandwidth = self.config.bandwidth
        model_size = 100000
        degree = model_size//bandwidth
        return degree
...
.format(self.i_episode, self.i_episode, self.round_num), 'w+') as gg:
    ...
    def federated_averaging(self, reports):
        ...
        # with open('./server/new_weight.txt','w+') as ne:
        #     ne.write(str(new_weight))
    ...
    def generate_comm_matrix(self, graph):
        ...
        return comm_matrix
    # 用 communication matrix 計算 node 數
    ...
    def take_action(self, state, is_testing = False):
        ...
        if ((rand_val > eps_threshold) or (is_testing == True)):
            ...
            action_list = torch.topk(val, len(val))[1] # 把 action 以 value 由大到小排列
            ########### 找沒用過的的最大 value 的 action 且符合band width 限制 #####
            # satisfy = False
            # for a in action_list:
            #     if (a.item() not in self.invalid_action):
            #         action = a.item()
            #         if action != 0: ## 如果動作非不變
            ...
            #             break
            ########################################################################
            ########### 不管你有沒有用過 反正就是加爆你 只要你符合 network 限制 #######################
            satisfy = False
            for a in action_list:
            ...
            while(True):
                ##########################################
                # action = np.random.\
                #     choice([i for i in range(0, self.num_action) if i not in self.invalid_action])
                ##########################################
                action = np.random.\
